...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-MP-PIX-SUBSCRIPTION] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Create Mercado Pago checkout preference for ONE-TIME payment (PIX)
    const preferenceData = {
      items: [
        {
          title: `${plan.name} - 1 mês`,
          description: `Assinatura mensal do plano ${plan.name} (pagamento único via PIX)`,
          quantity: 1,
      ...
      payment_methods: {
        // Allow PIX and other one-time payment methods
      ...
      back_urls: {
        success: `${origin}/dashboard/plans?subscription=pending&plan=${planKey}&payment_type=pix`,
        failure: `${origin}/dashboard/plans?subscription=failed&plan=${planKey}`,
        pending: `${origin}/dashboard/plans?subscription=pending&plan=${planKey}&payment_type=pix`,
      },
    ...
    logStep("Creating MP preference for PIX", { reason: preferenceData.items[0].title, amount: plan.price });
    ...
    logStep("MP preference created", { preferenceId: mpData.id, initPoint: mpData.init_point });
