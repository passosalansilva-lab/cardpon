-- Update the trigger to generate lottery tickets when order is CONFIRMED (not delivered)
...
-- Rename the trigger for clarity (it still fires on UPDATE, but now looks for 'confirmed')
DROP TRIGGER IF EXISTS generate_lottery_tickets_on_delivery ON public.orders;
CREATE TRIGGER generate_lottery_tickets_on_confirm
AFTER UPDATE ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.generate_lottery_tickets();

-- Also create an INSERT trigger for orders that are created already confirmed (e.g., from POS)
DROP TRIGGER IF EXISTS generate_lottery_tickets_on_insert ON public.orders;
CREATE TRIGGER generate_lottery_tickets_on_insert
AFTER INSERT ON public.orders
FOR EACH ROW
WHEN (NEW.status = 'confirmed' AND NEW.customer_id IS NOT NULL)
EXECUTE FUNCTION public.generate_lottery_tickets();