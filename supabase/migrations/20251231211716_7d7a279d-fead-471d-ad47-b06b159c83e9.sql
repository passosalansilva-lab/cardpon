-- Função para verificar se uma empresa tem acesso a uma feature
-- Verifica: 1) Se está no plano da empresa, ou 2) Se foi comprada/concedida individualmente
...
AS $$
  SELECT EXISTS (
    -- Verificar se a feature está incluída no plano da empresa
    SELECT 1
    FROM companies c
    JOIN subscription_plans sp ON sp.key = c.subscription_plan
  ...
  OR EXISTS (
    -- Verificar se a feature foi comprada/concedida individualmente
    SELECT 1
    FROM companies c
  ...
  OR EXISTS (
    -- Verificar se é staff de uma empresa com acesso
    SELECT 1
